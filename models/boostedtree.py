    ...
    def fit(self):
        ...
        for quantile in self.quantiles:
            ...
            model = GradientBoostingRegressor(
                loss="quantile", alpha=quantile, n_estimators=100)
            ...
            print(
                f"Training Successful for ALL STOCKS from {merged_stock_df['date'].min()} to {merged_stock_df['date'].max()}! HURRAYYY !")
    ...
    def predict(self, periods_ahead, window_size=50):
        ...
        for permno, date, predicted_returns in zip(merged_stock_df['permno'], merged_stock_df['date'], quantile_predictions[0.5]):
            ...
            prediction_date = first_permno_price[str(
                date) + ',' + str(permno)]['last_date'] + timedelta(self.options['prediction_offset'])
