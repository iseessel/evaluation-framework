...
class FBProphet:
  ...
  """
    ...
    def fit(self):
        ...
        prophet_df.rename(
            columns={'adjusted_prc': 'y', 'date': 'ds'}, inplace=True)
        prophet_df['ds'] = pd.to_datetime(prophet_df['ds'])

        # Set prediction interval to 95%
        self.trained_model = Prophet(
            **self.hypers, interval_width=0.95).fit(prophet_df)
        return self
    """
  ...
  """
    ...
    def predict(self, periods_ahead, features=None):
        ...
        future = self.trained_model.make_future_dataframe(
            periods=sorted(periods_ahead)[-1], include_history=False, freq='D')
