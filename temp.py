...
class NonLinearOptimization:
    ...
    def __get_predicted_returns(self):
        ...
        df['Expected Return'] = (
            self.predictions['adjusted_prc_pred'] / self.predictions['adjusted_prc_train_end']) - 1
        df['Standard Deviation'] = (
            self.predictions['std_dev_pred'] / self.predictions['adjusted_prc_train_end'])
    ...
    def __create_correlation_matrix(self):
        ...
        df = query[query['permno'].isin(
            available_permnos)].sort_values(by='permno')

        df = df.pivot_table(index='date', columns='permno',
                            values='adjusted_prc').reset_index()
...
kwargs = {
    ...
    'dataset': 'silicon-badge-274423.stock_model_evaluation.fb_prophet_sp_daily_features_v0_prod',
